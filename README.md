...
- `scan()` — Запустить получение следующего пакета без задержки

События:

- `data` — Пакет получен
- `scan` — Запущено получение следующего пакета без задержки
...
└── lib/
    ...
    ├── sleep.js             # Работа с задержками
    │
    ├── addon/
    │   └── l2raw.c          # Исходник C-аддона
    │
